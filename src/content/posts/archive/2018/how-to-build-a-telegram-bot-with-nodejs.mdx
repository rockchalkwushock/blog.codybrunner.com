...
archived: true
author: Cody Brunner
canonicalURL: https://blog.codybrunner.com/archive/2018/how-to-build-a-telegram-bot-with-nodejs
categories:
  - technology
createdAt: 02/11/2018
description: My experience in building a bot for the popular Telegram Messenger Platform.
draft: false
featured: false
heroImage:
  alt: Photo by SVGPorn
  src: https://cdn.svgporn.com/logos/nodejs.svg
language: 'en'
publishedAt: 02/11/2018
tags:
  - ci-cd
  - nodejs
...
import image from '../../../../assets/images/2018/how-to-build-a-telegram-bot-with-node-js/info.png'
import imageTwo from '../../../../assets/images/2018/how-to-build-a-telegram-bot-with-node-js/start_command.png'
import imageThree from '../../../../assets/images/2018/how-to-build-a-telegram-bot-with-node-js/tell_me_a_joke.png'
...
Micro-bot requires that you pass it a **token** and a **domain** for the bot to run on and these are already recognized as `process.env.BOT_TOKEN` and `process.env.BOT_DOMAIN`. In this tutorial we are going to deploy from [CircleCi](https://circleci.com) to the [Now](https://circleci.com) platform.
...
Specifically Telegram's API states that users will see a _Start Button_ to begin interacting with your bot and that these [global commands](https://core.telegram.org/bots#global-commands) are available to you for use by default. We will send the user a start up message when they tap the start button that will tell the user who our bot is and what it does.
...
app.command('start', ({ botInfo, replyWithMarkdown, update }) =>
  replyWithMarkdown(
    ...
    Markup.keyboard(['Tell me something funny Chuck']).resize().extra()
  )
)
```

<div class='flex flex-col items-center justify-center bg-aura-black'>
	<img alt='Start Interaction with the bot.' class='h-96' src={imageTwo} />
	<small class='text-aura-gray'>Get started talking with Chuck.</small>
</div>

## Listening for the user's response

Now we just need to implement a listener for the user's response of asking for a joke. This is where our function for making an API call to the Chuck Norris API will come into play. Since the function is an _asynchronous_ function we will make sure to use the `async/await` syntax here:
...
<div class='flex flex-col items-center justify-center bg-aura-black'>
	<img alt='Joke Time!' class='h-96' src={imageThree} />
	<small class='text-aura-gray'>Tell me a joke Chuck!</small>
</div>

## Adding information about your bot

Through @BotFather we can use commands to add information about our bot such as `/setdescription`, `/setabouttext`, `/setuserpic`, etc.

<div class='flex flex-col items-center justify-center bg-aura-black'>
	<img alt='Information about the bot.' class='h-96' src={image} />
	<small class='text-aura-gray'>Screen with all the info about Chuck.</small>
</div>

## Deploying the bot

I decided to write tests that can be found [here](https://github.com/rockchalkwushock/telegram_chuck_norris_bot/tree/master/__tests__) and use CircleCi to handle the deployment of the bot. Providing the `BOT_TOKEN` & `NOW_TOKEN` to CircleCi we can easily kickback and let it manage making sure our bot is ready for production before shipping. The full `config.yaml` can be found [here](https://github.com/rockchalkwushock/telegram_chuck_norris_bot/blob/master/.circleci/config.yml).

```yaml:.circleci/config.yaml
deploy_bot:
  docker:
...
Give ol'Chucky a try if you are on Telegram [@ChuckNorrisJokeBot](https://telegram.me/ChuckNorrisJokeBot)! 🤘🏻
