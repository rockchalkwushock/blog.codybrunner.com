...
author: cody-brunner
canonicalURL: >-
  https://blog.codybrunner.com/2022/a-far-too-in-depth-guide-to-ssh-for-web-developers
categories:
  - technology
createdAt: 01/07/2022
description: >-
  Everything I have learned when it comes to SSH after setting up my Raspberry
  Pi Cluster.
draft: false
featured: false
...
```ssh-config showLineNumbers title="~/.ssh/config"
Host *
  AddKeysToAgent yes
  UseKeychain yes
...
```ssh-config showLineNumbers title="/etc/ssh/sshd_config"
Port 222
AllowUsers cody
PermitRootLogin no
