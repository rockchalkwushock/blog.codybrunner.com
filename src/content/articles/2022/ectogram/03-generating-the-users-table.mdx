...
archived: true
author: cody-brunner
canonicalURL: 'https://blog.codybrunner.com/2022/ectogram/03-generating-the-users-table'
categories:
  - technology
coverImage:
  alt: Photo from Medium
  src: 'https://miro.medium.com/v2/resize:fit:1400/1*iFEiMUdRq3gb-bb3z6uh8g.png'
createdAt: 01/14/2022
description: >-
  Part 3 in the Ectogram series where I generate the user table migration and
  schema.
draft: false
featured: false
...
```elixir showLineNumbers title="priv/repo/migrations/2022019111111_add_users.exs"
...
```diff showLineNumbers title="priv/repo/migrations/2022019111111_add_users.exs"
defmodule Ectogram.Repo.Migrations.AddUsers do
  use Ecto.Migration
...
> For anyone not familiar with that SQL what is happening is we are passing the given `birth_date` to the [`AGE`](https://www.psqltutorial.com/psql-age/) function in Postgres. This returns an object with keys like `year`, `month`, etc where the values are numbers. The [`DATE_PART`](https://www.psqltutorial.com/psql-date_part/) function then cherry picks the desired part of the object for us. An example would be my birth year would evaluate to 33 when passed through these functions and then ran against the `>= 18` comparison. Postgres has some pretty slick features!
...
```elixir showLineNumbers title="lib/ectogram/user.ex"
...
```diff showLineNumbers title=".iex.exs"
- alias Ectogram.{Repo}
...
We will need to add one dependency before moving forward. We will be making use of the popular [`bcrypt`](https://hexdocs.pm/bcrypt_elixir/api-reference.html) password hashing library. Add the following to your `mix.exs` and then install the dependency:

```diff showLineNumbers title="mix.exs"
defp deps do
    [
      {:ecto_sql, "~> 3.0"},
-     {:postgrex, ">= 0.0.0"}
...
```elixir showLineNumbers title="priv/repo/seeds.exs"
alias Ectogram.{Repo, User}
import Faker
