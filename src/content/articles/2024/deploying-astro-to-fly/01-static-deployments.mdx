...
relatedArticles:
  ...
  - 2024/certificate-issues-between-fly-and-turso
...
In the `Dockerfile` we are going to make use of [GoStatic](https://hub.docker.com/r/pierrezemb/gostatic/), a static web server written in Go, to serve the static files that are output from the build step.
