...
archived: true
author: cody-brunner
canonicalURL: https://blog.codybrunner.com/archive/2017/rebuilding-my-site-with-hugo
categories:
  - technology
coverImage:
  alt: Photo by SVGPorn
  src: https://cdn.svgporn.com/logos/hugo.svg
createdAt: 12/18/2017
description: What I learned while rebuilding my site/blog with Hugo.
draft: false
featured: false
publishedAt: 12/18/2017
tags:
  - hugo
...
`hugo` bases everything it does off of the `config.<toml|yaml|json>`. I personally like `yaml` syntax better so I went with that. `hugo` has some predefined keys that you can find [here](https://gohugo.io/getting-started/configuration/). This file basically holds the default variables you set for your site. You can create custom variables and should place them under `.Site.Params.<your-var-here>`. I say this because `hugo` does have some other keys defined already like `menu`, `params`, & `social`. They are all there for their own reason so saying something like `menu: true` will make `hugo`’s internals get really angry with you 😂
...
```jsx showLineNumbers title="components/Post.jsx"
const Post props =>
  <div>
    <h2>{props.title}</h2>
    <h5>{props.date}</h5>
...
```html showLineNumbers title="_default/single.html"
<div>
	<h2>{{ .Title }}</h2>
	<h5>{{ .Date }}</h5>
	<p>{{ .Content }}</p>
...
```yml showLineNumbers title="content/posts/rewriting-my-site-with-hugo.md"
---
title: 'Rewriting My Site With Hugo'
createdAt: 2017-12-14T09:13:16-06:00
...
```jsx showLineNumbers title="components/ProjectList.jsx"
const list = [
	{ id: 1, name: 'a' },
	{ id: 2, name: 'b' },
	{ id: 3, name: 'c' },
...
const ProjectList = ({ list }) => (
	<ul>
		{list.map(prj => (
			<li key={prj.id}>{prj.name}</li>
		))}
	</ul>
)
```

```html showLineNumbers title="layouts/section/posts.html"
