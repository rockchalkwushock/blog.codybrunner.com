...
archived: true
author: cody-brunner
canonicalURL: https://blog.codybrunner.com/archive/2022/ectogram/02-setting-up-ecto
categories:
  - technology
coverImage:
...
relatedArticles:
  - archive/2022/ectogram/01-introduction
  - archive/2022/ectogram/03-generating-the-users-table
  - archive/2022/ectogram/04-testing-ecto
tags:
  - ecto
  - elixir
  - psql
...
I am assuming if you are following along with this series that you have `elixir`, `erlang`, & `psql` installed on your machine. Please refer back to the [intro post](https://codybrunner.com/blog/2022/ectogram/01-introduction) for the versions this post is following. With that let's dive in!
...
```elixir showLineNumbers title="mix.exs"
defp deps do
  [
    {:ecto_sql, "~> 3.0"},
    {:postgrex, ">= 0.0.0"}
...
```elixir showLineNumbers title="lib/ectogram/application.ex"
...
```elixir showLineNumbers title="config/config.exs"
import Config
...
```elixir showLineNumbers title="config/dev.exs"
import Config
...
```elixir showLineNumbers title="config/test.exs"
import Config
...
```diff showLineNumbers title="config/config.exs"
import Config
...
```elixir showLineNumbers title="lib/ectogram/schema.ex"
...
```elixir showLineNumbers title="test/support/data_case.ex"
...
```diff showLineNumbers title="test/test_helper.exs"
ExUnit.start()
...
```elixir showLineNumbers title=".iex.exs"
alias Ectogram.{Repo}
